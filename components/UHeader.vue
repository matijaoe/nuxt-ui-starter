    ...
    <UContainer class="w-full mx-auto px-4 sm:px-6 lg:px-8 flex items-center justify-between gap-3 h-[--header-height]">
      ...
      <ul class="items-center gap-x-8 hidden lg:flex">
        <li v-for="(link, i) in links" :key="i">
          <NuxtLink active-class="text-primary" :href="link.to"
      ...
      <div class="flex items-center justify-end lg:flex-1 gap-1.5">
        ...
        <button type="button"
          ...
          aria-label="Open Menu" @click="mobileMenuOpen = !mobileMenuOpen">
          <span class="i-ph-list-duotone flex-shrink-0 h-5 w-5"
            :class="mobileMenuOpen ? 'i-ph-x-duotone' : 'i-ph-list-duotone'" aria-hidden="true" />
        </button>
      </div>
    </UContainer>
  </header>
